...
export default function DashboardPage() {
  ...
  async function loadStats() {
    ...
    const { upcoming, past } = splitEvents(eventsRes.data ?? [])
    const singles = (releasesRes.data ?? []).filter(r => r.type === 'single').length
    const eps = (releasesRes.data ?? []).filter(r => r.type === 'ep').length
    const albums = (releasesRes.data ?? []).filter(r => r.type === 'album').length

    const avalsData = avalsRes.data ?? []
    const avgNota = avg(avalsData.map(a => a.nota_final).filter(n => n > 0))
