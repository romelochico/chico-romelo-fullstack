...
const Card = styled.div<{ $alert?: boolean }>`
  ...
  border: 1px solid
    ${({ $alert }) => ($alert ? 'rgba(248,113,113,0.35)' : 'rgba(255,255,255,0.13)')};
  border-radius: 12px;
  padding: 24px;
  display: flex;
  ...
  svg {
    color: ${({ $alert }) => ($alert ? '#f87171' : '#c8a96e')};
    width: 22px;
    height: 22px;
  }
...
const SubStat = styled.span<{ $green?: boolean; $red?: boolean }>`
  ...
  strong {
    font-weight: 700;
    color: ${({ $green, $red }) =>
      $red ? '#f87171' : $green ? '#4ade80' : 'rgba(245,240,232,0.85)'};
  }
...
export default function DashboardPage() {
  ...
  async function loadStats() {
    ...
    const singles = (releasesRes.data ?? []).filter(
      (r: { type: string }) => r.type === 'single'
    ).length
    const eps = (releasesRes.data ?? []).filter((r: { type: string }) => r.type === 'ep').length
    const albums = (releasesRes.data ?? []).filter(
      (r: { type: string }) => r.type === 'album'
    ).length

    const avalsData = (avalsRes.data ?? []) as { nota_final: number | null }[]
    const avgNota = avg(
      avalsData.map(a => a.nota_final).filter((n): n is number => n !== null && n > 0)
    )
    ...
    setStats({
      ...
      inventoryUnits: (inventoryRes.data ?? ([] as { quantity: number }[])).reduce(
    ...
    () => [
      {
        ...
        sub: s ? (
          <CardSub>
            <SubStat $green>
              <strong>{s.upcomingCount}</strong> próximos
            </SubStat>
            <SubStat>
              <strong>{s.pastCount}</strong> passados
            </SubStat>
          </CardSub>
        ) : null,
      },
      ...
      },
      {
        ...
        sub: s ? (
          <CardSub>
            <SubStat>
              <strong>{s.singles}</strong> singles
            </SubStat>
            <SubStat>
              <strong>{s.eps}</strong> EPs
            </SubStat>
          </CardSub>
        ) : null,
      },
      {
        ...
        sub: s ? (
          <CardSub>
            <SubStat>
              <strong>{s.showsCount}</strong> shows
            </SubStat>
            <SubStat>
              <strong>{s.avalsCount}</strong> avaliações
            </SubStat>
          </CardSub>
        ) : null,
      },
      {
        ...
        sub:
          s && s.contatosUnread > 0 ? (
            <CardSub>
              <SubStat $green>
                <strong>{s.contatosUnread}</strong> não lida{s.contatosUnread !== 1 ? 's' : ''}
              </SubStat>
            </CardSub>
          ) : null,
      },
      { href: '/admin/media', label: 'Imprensa', icon: Image, value: null, sub: null },
      {
        ...
        sub: s ? (
          <CardSub>
            <SubStat>
              <strong>{s.inventoryItems}</strong> tipo{s.inventoryItems !== 1 ? 's' : ''}
            </SubStat>
            <SubStat>
              <strong>{s.inventoryUnits}</strong> unidades
            </SubStat>
          </CardSub>
        ) : null,
      },
      {
        ...
        sub: s ? (
          <CardSub>
            <SubStat>
              <strong>{s.linksCount}</strong> links
            </SubStat>
            <SubStat>
              <strong>{s.credsCount}</strong> credenciais
            </SubStat>
          </CardSub>
        ) : null,
      },
      {
        ...
        sub:
          s && s.nextUrgentOpenCall ? (
            <CardSub>
              <SubStat $red>
                <strong>{s.nextUrgentOpenCall.name}</strong> fecha em {s.nextUrgentOpenCall.days}{' '}
                dia{s.nextUrgentOpenCall.days !== 1 ? 's' : ''}
              </SubStat>
            </CardSub>
          ) : null,
      },
