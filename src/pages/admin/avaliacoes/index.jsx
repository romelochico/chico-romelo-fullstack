...
export default function AvaliacoesPage() {
  ...
  const [globalStats, setGlobalStats] = useState({ totalShows: 0, totalAvals: 0, avgNota: 0 })
  const [loading, setLoading] = useState(true)

  const [dataShow, setDataShow] = useState('')
  const [nomeShow, setNomeShow] = useState('')
  const [localShow, setLocalShow] = useState('')
  const [papeis, setPapeis] = useState(new Set())
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      ;(avalsData ?? []).forEach(a => {
        ...
        if (a.nota_final > 0) perShow[a.show_id].notas.push(a.nota_final)
      })
      ...
      const allNotas = (avalsData ?? []).map(a => a.nota_final).filter(n => n > 0)
      setGlobalStats({
          ...
          {loading ? (
            ...
            <ShowsGrid>
              {[...shows].sort((a, b) => {
                ...
                if (sortBy === 'latest') return new Date(b.data_show) - new Date(a.data_show)
                if (sortBy === 'oldest') return new Date(a.data_show) - new Date(b.data_show)
          ...
          <CriarInner>
            ...
            <FieldGrid>
              <Field>
                ...
                <FieldInput id="data-show" type="date" value={dataShow} onChange={e => setDataShow(e.target.value)} />
              </Field>
              <Field>
                <FieldLabel htmlFor="nome-show">Nome do evento</FieldLabel>
                <FieldInput id="nome-show" type="text" placeholder="ex: São João, Festa X" value={nomeShow} onChange={e => setNomeShow(e.target.value)} />
              </Field>
              <FieldFull>
                <Field>
                  <FieldLabel htmlFor="local-show">Local</FieldLabel>
                  <FieldInput id="local-show" type="text" placeholder="ex: Parque da Paz, Almada" value={localShow} onChange={e => setLocalShow(e.target.value)} />
                </Field>
              </FieldFull>
            </FieldGrid>
